<template>
  <div>
    <div class="d-flex justify-content-between align-baseline campaine-details">
      <div class="info">
        <div class="title">
          <i class="icon ion-android-person">
            <strong>{{`${campaine.first_name} ${campaine.last_name}`}}</strong>
          </i>
        </div>
        ...
        <div v-for="(item, n) in info" :key="`info-${n}`">
          <i v-if="item.key !== 'status' && item.key !== 'email' && item.key !== 'created_at'" :class="item.icon">
            <strong>{{campaine[item.key]}}</strong>
          </i>
          ...
          <i v-else-if="item.key === 'email'" :class="item.icon">
            <a :href="`mailto:${campaine[item.key]}`">{{campaine[item.key]}}</a>
          </i>
    ...
    <div class="campaine-details">
      ...
      <div v-for="(item, n) in summary" :key="`summary-${n}`"
        ...
        <span v-if="item.title === 'Rating'" style="color: orangered">
          <i v-for="i in campaine[item.key]" :key="`star--${i}`" class="icon ion-star" />
        </span>
        <strong v-else v-html="campaine[item.key]" />
      </div>
      <md-divider class="md-hr md-theme-demo-light" />
    </div>
    <div v-if="social.find(k=> campaine[k.key]) ? true : false"
      class="campaine-details d-flex justify-content-center" style="height: 25px;">
      <div v-for="(item, n) in social" :key="`social-${n}`" style="width: 30px;">
        <i v-if="campaine[item.key] ? true : false" class="social-icon" :class="item.icon" :style="item.style"
          @click="goSocial(campaine[item.key])" />
      </div>
    </div>
    <md-divider class="md-hr md-theme-demo-light" />
  </div>
...
<script>
  export default {
    ...
    computed: {
      style() {
        let style = ''
        switch(this.$props.campaine.status) {
          case 'rejected':
      ...
      createdAt() {
        let date = new Date(this.$props.campaine.created_at)

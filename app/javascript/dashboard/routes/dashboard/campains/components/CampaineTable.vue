<template>
  <section class="campaines-table-wrap">
    <AgGridVue
  ...
  export default {
    ...
    watch: {
      wQuery(newValue, oldValue) {
        this.$store.dispatch('enCampaines/search', newValue).then(() => {
          this.updateRowData(this.campaines)
        })
      }
    },
    data: () => {
    ...
    methods: {
      onGridReady(params) {
        ...
        this.updateRowData(this.$props.campaines)
      },
      updateRowData(data) {

    ...
    <div class="table-actions-wrap">
      ...
      <div class="right-aligned-wrap">
        <div class="search-wrap">
          <md-autocomplete
            class="search"
            v-model="campaineState.query"
            :md-options="campaineState.queryHistory"
            md-layout="box"
            style="padding-bottom: 0;"
          >
            <label>{{$t('LEADS_PAGE.SEARCH_INPUT_PLACEHOLDER')}}</label>
          </md-autocomplete>
          <md-button class="md-info md-raised md-dense" @click="createCampaine">
...
<script>
  ...
  export default {
    ...
    methods: {
      createCampaine() {
        Promise.all([
          this.$store.dispatch('enCampaines/editID', 'new'),
          this.$store.dispatch('enCampaines/campaine', {id: 'new'}),
          this.$store.dispatch('enCampaines/editing', true)
        ])
      },
    },
  };

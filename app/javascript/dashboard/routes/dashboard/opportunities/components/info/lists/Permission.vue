<template>
  <div class="md-layout opportunities-lists-permission">
    <div v-for="(item, n) in options" :key="`permission-${n}`" class="check--item">
      ...
      <div v-if="item.id === 3 && item.checked">
        <form ref="opportunity_permission_form">
          <div class="md-layout">
            <multiselect :max-height="150" style="color: black;" v-model="opportunityState.opportunity.share_users"
              placeholder="Share Users.."
              label="email" track-by="id"
              :multiple="true" :taggable="true"
              :options="adUserState.users.filter(k => k.id !== opportunityState.opportunity.id)"
            />
          </div>
          <div class="md-layout">
            <multiselect :max-height="150" style="color: black;" v-model="opportunityState.opportunity.share_groups"
...
<script>
  ...
  export default {
    name: 'opportunities-lists-permission',
    props: {
      sending: {
        type: Boolean,
        default: false
...
<style lang="scss" scoped>
  .opportunities-lists-permission {
    .check--item {
      width: 100%;
      &--name {
        color: #DD2C00;

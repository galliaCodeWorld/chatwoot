    ...
    <div class="table-actions-wrap">
      ...
      <div class="right-aligned-wrap">
        <div class="search-wrap">
          <md-autocomplete
            class="search"
            v-model="opportunityState.query"
            :md-options="opportunityState.queryHistory"
            md-layout="box"
            style="padding-bottom: 0;"
          >
            <label>{{$t('LEADS_PAGE.SEARCH_INPUT_PLACEHOLDER')}}</label>
          </md-autocomplete>
          <md-button class="md-info md-raised md-dense" @click="createOpportunity">
...
<script>
  ...
  export default {
    ...
    methods: {
      createOpportunity() {
        Promise.all([
          this.$store.dispatch('enOpportunities/editID', 'new'),
          this.$store.dispatch('enOpportunities/opportunity', {id: 'new'}),
          this.$store.dispatch('enOpportunities/editing', true)
        ])
      },
    },
  };

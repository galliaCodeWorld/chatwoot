<template>
  <section class="opportunities-table-wrap">
    <AgGridVue
  ...
  export default {
    ...
    watch: {
      wQuery(newValue, oldValue) {
        this.$store.dispatch('enOpportunities/search', newValue).then(() => {
          this.updateRowData(this.opportunities)
        })
      }
    },
    data: () => {
    ...
    methods: {
      onGridReady(params) {
        ...
        this.updateRowData(this.$props.opportunities)
      },
      updateRowData(data) {

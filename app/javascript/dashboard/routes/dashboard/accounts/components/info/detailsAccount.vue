<template>
  <div>
    <div class="d-flex justify-content-between align-baseline account-details">
      <div class="info">
        <div class="title">
        ...
        <div v-for="(item, n) in info" :key="`info-${n}`">
          <i v-if="item.key !== 'status' && item.key !== 'email' && item.key !== 'created_at'" :class="item.icon">
            <strong>{{account[item.key]}}</strong>
          </i>
          ...
          <i v-else-if="item.key === 'email'" :class="item.icon">
            <a :href="`mailto:${account[item.key]}`">{{account[item.key]}}</a>
          </i>
    ...
    <div class="account-details">
      ...
      <div v-for="(item, n) in summary" :key="`summary-${n}`"
        ...
        <span v-if="item.title === 'Rating'" style="color: orangered">
          <i v-for="i in account[item.key]" :key="`star--${i}`" class="icon ion-star" />
        </span>
        <strong v-else v-html="account[item.key]" />
      </div>
      <md-divider class="md-hr md-theme-demo-light" />
    </div>
    <div v-if="social.find(k=> account[k.key]) ? true : false"
      class="account-details d-flex justify-content-center" style="height: 25px;">
      <div v-for="(item, n) in social" :key="`social-${n}`" style="width: 30px;">
        <i v-if="account[item.key] ? true : false" class="social-icon" :class="item.icon" :style="item.style"
          @click="goSocial(account[item.key])" />
      </div>
    </div>
    <md-divider class="md-hr md-theme-demo-light" />
  </div>
...
<script>
  export default {
    ...
    computed: {
      ...
      createdAt() {
        let date = new Date(this.$props.account.created_at)

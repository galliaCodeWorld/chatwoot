<template>
  <div class="md-layout accounts-lists-permission">
    <div v-for="(item, n) in options" :key="`permission-${n}`" class="check--item">
      ...
      <div v-if="item.id === 3 && item.checked">
        <form ref="account_permission_form">
          <div class="md-layout">
            <multiselect :max-height="150" style="color: black;" v-model="accountState.account.share_users"
              placeholder="Share Users.."
              label="email" track-by="id"
              :multiple="true" :taggable="true"
              :options="adUserState.users.filter(k => k.id !== accountState.account.id)"
            />
          </div>
          <div class="md-layout">
            <multiselect :max-height="150" style="color: black;" v-model="accountState.account.share_groups"
...
<script>
  ...
  export default {
    name: 'accounts-lists-permission',
    props: {
      sending: {
        type: Boolean,
        default: false
